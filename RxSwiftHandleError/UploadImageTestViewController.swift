    ...
    override func viewDidLoad() {
        ...
        uploadImageButton.rx.tap.asDriver()
            .flatMap { [unowned self] () -> Driver<()> in
                Observable
                    .deferred({ () -> Observable<Int> in
                       //["Han", "Luke", "Chewie"].random(using: &ARC4Random.default)
                        //Int.random(in: -100 ... 100, using: &randomGenerator)
                        //[].random(using: RandomGenerator)
                        let url = URL(string: "https://httpbin.org/status/\([200, 503, 599].random(using: &ARC4Random.default)!)?type=upload")!
                        //let url = URL(string: "https://httpbin.org/status/200?type=upload")!
            ...
            .flatMap { [unowned self] () -> Driver<()> in
                Observable
                    .deferred({ () -> Observable<Int> in
                        let url = URL(string: "https://httpbin.org/status/\([200, 503, 408].random(using: &ARC4Random.default)!)?type=request")!
                        //let url = URL(string: "https://httpbin.org/status/200?type=request")!
                        return URLSession.shared.rx.statusCode(url: url)
                    })
                    .map { _ in }
                    .trackActivity(activityIndicator)
